...
def query_instrument_id(symbol, contract):
    ...
    if which_api == '':
        ...
        which_api_2 = spot.SpotAPI(api_key, secret_key, passphrase, False)
...
def query_ticker(instrument_id):
    ticker = dict()
    try:
        ticker = which_api_2.get_specific_ticker(instrument_id)
    except Exception as ex:
        logging.info('%s %s' % (instrument_id, ex))
    # print (ticker)
    return ticker
...
# lever order:
# {"client_oid":"yt20180728","order_type”:”1”,"instrument_id":"btc-usdt","side":"buy","type":"market","size":"0.1","notional":"100","margin_trading ":"2"}
# otype = 'buy' | 'sell'
